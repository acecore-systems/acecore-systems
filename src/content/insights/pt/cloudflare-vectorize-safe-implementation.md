...
linkCards:
  - href: /insights/cloudflare-vectorize-implementation-guide/
    title: Entender primeiro os papéis de Vectorize e RAG
    description: "Uma introdução curta à pesquisa semântica, ao RAG e às condições necessárias para respostas com fontes."
    icon: i-lucide-route
...
1. Criar um index de substituição compatível com o novo contrato de modelo, dimensions e metric.
2. Sincronizar integralmente o corpus publicado e verificar a convergência do conjunto de IDs e canaries de queries conhecidas.
3. Trocar o binding de Worker ou Pages para o index de substituição.
4. Verificar a API de busca de Production, o fallback de busca comum e o binding ativo.
5. Excluir o index antigo somente com uma aprovação explícita separada.
