...
faq:
  ...
  items:
    - question: Почему не отправлять целевую услугу в hidden-поле?
      answer: "В июне 2026 года обращения классифицировали только по существующей категории, без дополнительных скрытых полей. Текущая форма Systems также сохраняет service, from и entry в скрытых полях для интеграции."
...
> **Обновление за сентябрь 2026 года:** Примеры `ServiceSectionActions` и `service=web` описывают схему корпоративного сайта июня 2026 года. Сейчас [CTA услуг Acecore Systems](https://github.com/acecore-systems/acecore-systems/blob/main/src/components/ServiceDetailPage.astro) передают `category`, `service`, `from` и `entry`, а [форма связи](https://github.com/acecore-systems/acecore-systems/blob/main/src/pages/contact.astro) проверяет короткий ключ услуги и выбирает категорию и тему. Для аналитики и интеграции используются скрытые поля, поэтому старую схему «без скрытых полей» нельзя копировать как действующую. Значения URL по-прежнему нужно сопоставлять с разрешёнными вариантами формы.
