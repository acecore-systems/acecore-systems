...
**Обновление от 26 сентября 2026 года:** Описанный ниже вход через GitHub OAuth Worker относится к первоначальной схеме. В коде, объединённом в сентябре, корпоративный сайт проверяет личность AcecoreID / Cloudflare Access, привязанный GitHub ID и право записи непосредственно перед сохранением. Отдельная GitHub App по-прежнему записывает данные в репозиторий после проверки путей, содержимого и текущего HEAD и создаёт commit напрямую в `main`. OpenAI Batch и PR переводов остаются отдельным путём. См. [объединённый PR #251](https://github.com/acecore-systems/acecore-net/pull/251).
