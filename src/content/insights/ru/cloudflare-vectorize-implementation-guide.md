...
callout:
  type: tip
  title: Поиск — fail-soft, синхронизация и публикация — fail-closed
  text: "Если Vectorize недоступен, пользовательский поиск сохраняет Pagefind. Но синхронизация index и публикация в Production останавливаются, если нельзя подтвердить целевое окружение, corpus, долю удалений, опубликованный commit или завершение mutation. При переносе на несколько сайтов именно эта асимметричная архитектура оказалась наиболее полезной."
processFigure:
...
После внедрения или тестирования Cloudflare Vectorize в нескольких репозиториях становится ясно: недостаточно просто «создать embedding и вызвать `query()`».

Как формировать данные для поиска? Как сохранить существующий поиск? Как разделить Preview и Production? Как не допустить массового удаления при ошибочной синхронизации? Действительно ли index соответствует опубликованным сейчас страницам? В эксплуатации архитектура до и после вызова Vectorize API оказалась важнее самого вызова.

В этой статье сопоставлены результаты внедрения и исследования в Acecore Systems, World Foundation, Acecore Schools и Aceserver Portal и выделен подход, который можно повторно использовать на других сайтах Astro／Cloudflare Pages.
