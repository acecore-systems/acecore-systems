...
linkCards:
  - href: /insights/cloudflare-vectorize-implementation-guide/
    title: 先理解 Vectorize 与 RAG 的角色
    description: "简要说明语义搜索、RAG 的区别，以及带有出处的回答所需条件。"
    icon: i-lucide-route
...
1. 创建符合新 model、dimensions 和 metric 契约的替换 index。
2. 全量同步已发布 corpus，并确认 ID 集合收敛以及已知 query 的 canary。
3. 将 Worker 或 Pages binding 切换到替换 index。
4. 验证 Production 搜索 API、普通搜索 fallback 以及在线 binding。
5. 仅在单独的明确批准后删除旧 index。
