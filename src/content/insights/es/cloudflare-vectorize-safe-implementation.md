...
linkCards:
  - href: /insights/cloudflare-vectorize-implementation-guide/
    title: Comprender primero las funciones de Vectorize y RAG
    description: "Una introducción breve a la búsqueda semántica, RAG y las condiciones necesarias para respuestas con fuentes."
    icon: i-lucide-route
...
## Usar un index de reemplazo ante una eliminación grande o un cambio de modelo

Una eliminación superior al 20 % no debe quedar dentro de la sincronización diferencial ordinaria. Ese 20 % no es un límite de producto de Cloudflare: es una barrera operativa que detiene el flujo habitual para revisión humana.

Cuando se esperaba eliminar el 21,3 % de los vectores de un index existente, no borramos directamente. Cambiamos a esta secuencia:

1. Crear un index de reemplazo que coincida con el nuevo contrato de modelo, dimensiones y métrica.
2. Sincronizar por completo el corpus publicado y comprobar la convergencia del conjunto de IDs y canaries con queries conocidas.
3. Cambiar el binding de Worker o Pages al index de reemplazo.
4. Verificar la API de búsqueda de Production, el fallback de la búsqueda ordinaria y el binding activo.
5. Eliminar el index antiguo solo con una aprobación explícita independiente.
