...
linkCards:
  - href: /insights/cloudflare-vectorize-implementation-guide/
    title: Comprendre d'abord les rôles de Vectorize et du RAG
    description: "Une brève introduction à la recherche sémantique, au RAG et aux conditions nécessaires pour des réponses sourcées."
    icon: i-lucide-route
...
## Utiliser un index de remplacement lors d'une suppression importante ou d'un changement de modèle

Une suppression de plus de 20 % ne doit pas rester dans une synchronisation différentielle ordinaire. Ces 20 % ne sont pas une limite produit de Cloudflare : c'est un garde-fou opérationnel qui arrête le flux habituel pour une revue humaine.

Lorsqu'un index existant devait perdre 21,3 % de ses vecteurs, nous n'avons pas supprimé directement. Nous avons suivi cette séquence :

1. Créer un index de remplacement correspondant au nouveau contrat de modèle, de dimensions et de métrique.
2. Synchroniser intégralement le corpus publié, puis vérifier la convergence de l'ensemble des IDs et des canaries avec des requêtes connues.
3. Basculer le binding Worker ou Pages vers l'index de remplacement.
4. Vérifier l'API de recherche de Production, le fallback de recherche ordinaire et le binding actif.
5. Ne supprimer l'ancien index qu'après une approbation explicite distincte.
