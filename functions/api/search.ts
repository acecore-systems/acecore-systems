...
const CLIENT_ID_HEADER = "X-Acecore-Search-Client";
...
export const onRequestPost: PagesFunction<CloudflareEnv> = async (context) => {
  ...
  try {
    ...
    if (
      ...
      Number(OPENAI_EMBEDDING_DIMENSIONS) !== EMBEDDING_DIMENSIONS ||
      !SEARCH_INDEX ||
      !SEARCH_RATE_LIMIT_DB
    ) {
    ...
    if (requestId.endsWith("00")) {
      context.waitUntil?.(
        deleteExpiredRateLimits(SEARCH_RATE_LIMIT_DB).catch((error) => {
          logSearchError(
            requestId,
            SEARCH_LOCALE,
            "rate_limit_cleanup",
...
function normalizeLocale(value: unknown): typeof SEARCH_LOCALE | null {
  if (typeof value !== "string") return null;
...
async function createClientRateLimitKey(request: Request): Promise<string> {
  ...
  const source =
    connectingIp && connectingIp.length <= 64
      ? `ip:${connectingIp}`
      : `session:${normalizeClientId(request.headers.get(CLIENT_ID_HEADER))}`;
...
async function createOpenAiEmbedding(
  ...
  const abortFromRequest = () => timeoutController.abort(requestSignal.reason);
...
function normalizeMatches(
  ...
  for (const value of queryResult.matches) {
    ...
    const metadata = normalizeMetadata(value.metadata, requestUrl);
...
function normalizeMetadata(
  ...
  const contentType = /^[a-z0-9][a-z0-9_-]{0,39}$/i.test(contentTypeValue)
    ...
    : "page";
  const locale = readMetadataText(value.locale, 16).toLowerCase();

  if (!url || !title || locale !== SEARCH_LOCALE) return null;
...
function normalizePublicUrl(value: unknown, requestUrl: string): string | null {
  ...
  try {
    ...
    if (firstPathSegment && PRIVATE_PATH_SEGMENTS.has(firstPathSegment)) {
      return null;
    }
